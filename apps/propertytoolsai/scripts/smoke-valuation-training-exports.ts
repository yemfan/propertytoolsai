 ...
 *   npm run smoke:valuation-training-exports -w propertytoolsai
 *
 * Env: apps/propertytoolsai/.env.local — NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
 */
...
async function main() {
  ...
  if (!url || !key) {
    console.error("Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY (apps/propertytoolsai/.env.local)");
    process.exitCode = 1;
    return;
  }

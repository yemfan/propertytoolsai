-- =============================================================================
-- LEADS CRM — complete / repair script (run in Supabase SQL editor if needed)
-- =============================================================================
-- Problem: some migrations used lead_id UUID + FK → public.leads(id), but
-- many databases have public.leads.id as BIGINT. That causes 42804 errors.
--
-- Fix: store lead_id as TEXT everywhere (no FK). App/PostgREST matches strings
-- to bigint or uuid ids the same way.
--
...
-- -----------------------------------------------------------------------------
-- 1) Drop foreign keys on lead_id that reference public.leads (any type mismatch)
-- -----------------------------------------------------------------------------
do $$
declare
  r record;
...
-- -----------------------------------------------------------------------------
-- 2) Normalize lead_id column type → text (only when not already text)
-- -----------------------------------------------------------------------------
do $$
declare
  r record;
...
-- -----------------------------------------------------------------------------
-- 3) Create core CRM tables if missing (text lead_id, no FK)
-- -----------------------------------------------------------------------------
-- NOTE: IF NOT EXISTS skips when the table is already there. Older tables may
-- lack created_at while this script still creates indexes on (…, created_at).
-- That causes ERROR 42703. Section 3a+3b runs after all CREATEs to add/backfill
-- created_at before any index references it.
...
-- home_value_reports: lead_id optional
...
-- -----------------------------------------------------------------------------
-- 3a–3b) Ensure created_at on legacy rows, then indexes that reference it
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4) Leads table: scoring columns (safe add)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5) Optional columns added in later small migrations
-- -----------------------------------------------------------------------------

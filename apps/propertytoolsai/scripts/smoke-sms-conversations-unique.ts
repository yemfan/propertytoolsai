/**
 * Verifies 20260324_sms_conversations_unique.sql (unique index on sms_conversations.lead_id).
 *
 * Usage (from repo root):
 *   npm run smoke:sms-conversations-unique -w propertytoolsai
 *
...
async function main() {
  ...
  const { error: readErr } = await supabase.from("sms_conversations").select("id,lead_id").limit(1);
  if (readErr) {
  ...
  const { data: convRows, error: convErr } = await supabase.from("sms_conversations").select("lead_id");
  if (convErr) {
    console.error(`✗ sms_conversations list: ${convErr.message}`);
    process.exitCode = 1;
    return;
  }
  const used = new Set((convRows ?? []).map((r) => r.lead_id));

  const { data: leadRows, error: leadErr } = await supabase.from("leads").select("id").limit(200);
  if (leadErr) {
  ...
  const { error: ins1 } = await supabase
    ...
    .insert({ lead_id: freshLeadId, messages: [] });
  if (ins1) {
  ...
  await supabase.from("sms_conversations").delete().eq("lead_id", freshLeadId);

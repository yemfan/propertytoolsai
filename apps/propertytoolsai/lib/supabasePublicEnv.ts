...
export function requireSupabasePublicEnv(): { url: string; anonKey: string } {
  ...
  if (!env) {
    throw new Error(
      "Missing NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY. Copy apps/propertytoolsai/.env.example to .env.local in this app, then paste Project URL + anon public API key from Supabase → Project Settings → API: https://supabase.com/dashboard/project/_/settings/api"
    );
  }
  return env;
}

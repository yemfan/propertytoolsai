 ...
 * Does not require `SUPABASE_SERVICE_ROLE_KEY` on the API — fixes uploads when the server only has the anon key.
 */
export async function uploadProfilePhotoWithSessionClient(
  ...
  if (upErr) {
    ...
    if (/row-level security|rls|policy/i.test(msg)) {
      return {
        ok: false,
        error: `${msg} Run the latest Supabase migration that adds storage policies for the "avatars" bucket (authenticated upload to own folder).`,
      };
    }

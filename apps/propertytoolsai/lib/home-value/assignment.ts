...
export type PickedAgent = {
  /** Owning bigint id from public.agents — what contacts.agent_id wants. */
  id: number;
  /** Supabase auth UUID — what agent_notifications and follow-ups want. */
...
/**
 * Choose an active, accepting agent for a freshly-captured home-value
 * lead. Same matching logic as the legacy autoAssignLeadToAgent
 * (city-exact in service_areas_v2 → "all cities in county" entries →
 * loose service_areas array → first active agent fallback) but pure:
 * no DB writes, just returns the pick. Used by the home-value
 * unlock-report flow to satisfy contacts.agent_id NOT NULL at insert
 * time. Returns null only when zero accepting agents exist.
 */
export async function pickAgentForHomeValueLead(
  input: PickAgentInput,
): Promise<PickedAgent | null> {
  const { data: agents, error } = await supabaseAdmin
    .from("agents")
    .select("id, auth_user_id, service_areas, service_areas_v2, accepts_new_leads")
  ...
  let selected = agents.find((a) => {
    ...
    return areas.some(
      (area) => area === normalizedZip || area.toLowerCase() === normalizedCity,
    );
  });
  ...
  return {
    ...
    authUserId: (selected.auth_user_id as string | null) ?? null,
    fullName,
    email,
  };
}

...
// ---------------------------------------------------------------------------
// Conversion prediction (behavioral UserProfile from client buffer)
// ---------------------------------------------------------------------------

export type ConversionCategory = "low" | "medium" | "high";

export type ConversionPrediction = {
  /** 0–100 likelihood of converting to a lead / paid action */
  score: number;
...
/**
 * Predict conversion likelihood from a behavioral `UserProfile` (see `buildUserProfile`).
 * Complements CRM `scoreLead(leadId)` which uses DB lead_events.
 */
export function predictLeadScore(profile: UserProfile): ConversionPrediction {
  ...
  const add = (msg: string, pts: number) => {
    raw += pts;
    if (pts > 0) reasons.push(msg);
  };

  if (profile.urgency === "high") add("High urgency signal", 22);
  else if (profile.urgency === "medium") add("Medium urgency", 10);

  if (profile.intent === "investor") add("Investor intent pattern", 14);
  else if (profile.intent === "seller") add("Seller intent pattern", 12);
  else if (profile.intent === "buyer") add("Buyer intent pattern", 10);

  raw += Math.min((s.comparison_started ?? 0) * 12, 24);
  if ((s.comparison_started ?? 0) > 0) reasons.push("Started AI / multi-property comparison");

  raw += Math.min((s.cma_used ?? 0) * 10, 20);
  if ((s.cma_used ?? 0) > 0) reasons.push("CMA or pricing workflow");

  raw += Math.min((s.mortgage_used ?? 0) * 6, 12);
  if ((s.mortgage_used ?? 0) > 0) reasons.push("Mortgage / financing engagement");

  raw += Math.min((s.home_value_used ?? 0) * 5, 10);
  raw += Math.min((s.property_viewed ?? 0) * 3, 9);

  raw += Math.min((s.cap_rate_used ?? 0) * 5, 10);
  if ((s.cap_rate_used ?? 0) > 0) reasons.push("Investment calculators");

  if ((s.agent_clicked ?? 0) > 0) add("Clicked agent / expert CTA", 18);

  const toolDiversity = [
    s.comparison_started,
    s.cma_used,
    s.mortgage_used,
    s.home_value_used,
    s.cap_rate_used,
  ].filter((n) => (n ?? 0) > 0).length;
  if (toolDiversity >= 3) add("Multiple tool categories used", 12);

  if (profile.totalEvents >= 12) add("Sustained engagement", 8);
  else if (profile.totalEvents >= 6) add("Repeat visits / events", 4);

  if (profile.priceRange && profile.priceRange.max > 0) add("Price context captured", 4);

  const score = clampScore(raw);
  const category: ConversionCategory =
    score >= 71 ? "high" : score >= 40 ? "medium" : "low";

  return {
    score,
    category,
    reasons: reasons.length ? reasons : ["Early-stage or exploratory session"],
  };
}

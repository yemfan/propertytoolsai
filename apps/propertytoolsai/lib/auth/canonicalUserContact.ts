...
/**
 * Contact identity for PropertyTools: **Supabase Auth (`auth.users`) is the source of truth.**
...
/** IdP/OAuth claims (Google `name`, `given_name` + `family_name`, etc.) — not persisted as the canonical key. */
...
/**
 * Display name: canonical **`full_name`** in `user_metadata`, with legacy/provider fallbacks
 * (`name`, `given_name`, …) until OAuth callback or profile update normalizes storage.
 */
...
/**
 * After OAuth sign-in: if `full_name` is still empty, copy from provider claims into `full_name`
 * (via `updateUser({ data: { full_name } })`).
 */
...
/** Persist updates using **`full_name` only** — drops legacy duplicate `name` from metadata. */
...
/** Persist contact fields to `user_profiles` without clobbering avatar or other columns. */
...
/** US 10-digit → E.164 for Supabase `phone` on auth.users. */

...
import CmaComparisonCta from "@/components/cma/CmaComparisonCta";
import { trackEvent } from "@/lib/analyticsClient";
import { evaluateConversionOutreach } from "@/lib/conversionOutreachClient";
import { trackCmaUsed } from "@/lib/tracking";
...
function SmartCmaBuilderPageInner() {
  ...
  const { address, setAddress, saveSelectedAddress } =
    useAddressPrefill(queryAddress);
  ...
  const cmaCompletedTracked = useRef(false);
  ...
  useEffect(() => {
    if (!data) {
      cmaCompletedTracked.current = false;
      return;
    }
    if (cmaCompletedTracked.current) return;
    cmaCompletedTracked.current = true;
    void trackEvent("cma_completed", {
      source: "smart_cma_builder",
  ...
  const attachedLeadAddress = attachedLead?.property_address?.trim() ?? "";

  useEffect(() => {
    // When a lead is selected, prefill address only if the field is still empty.
    // Do NOT depend on `address` — when the user clears the field to type, we must not
    // re-run and overwrite (that broke Google address autocomplete on this page).
    // Use stable string `attachedLeadAddress` instead of `attachedLead` so refetches of
    // /api/leads don't replace the object reference and reset the input.
    if (!attachedLeadId || !attachedLeadAddress) return;
    setAddress((prev) => (prev.trim() ? prev : attachedLeadAddress));
  }, [attachedLeadId, attachedLeadAddress, setAddress]);
      ...
      <div className="bg-white shadow rounded-xl p-6 border border-gray-100">
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-start">
            <div className="relative z-30 min-w-0 w-full flex-1">
              <AddressAutocomplete
                value={address}
                onChange={setAddress}
                onBlur={() => {

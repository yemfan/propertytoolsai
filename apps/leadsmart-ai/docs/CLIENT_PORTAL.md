...
```bash
npm run smoke:client-portal -w leadsmart-ai
```

Requires `NEXT_PUBLIC_SUPABASE_URL` and `SUPABASE_SERVICE_ROLE_KEY` in `apps/leadsmart-ai/.env.local`. Confirms `client_portal_messages`, `client_saved_homes`, and `client_portal_documents` are readable.

...
/** `leads.agent_id` is bigint → `agents.id`. Query param is often Supabase auth UUID (`agents.auth_user_id`). */
...
export async function POST(req: Request) {
  try {
    ...
    //    Omit lead_id here and link via `leads.report_id` below to avoid insert failures.
    ...
    const brandName = process.env.AGENT_BRAND_NAME || "PropertyTools AI";

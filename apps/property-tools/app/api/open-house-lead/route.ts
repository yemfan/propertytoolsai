...
export async function POST(req: Request) {
  try {
    ...
    if (leadErr || !lead?.id) {
      return NextResponse.json(
        { success: false, message: leadErr?.message || "Failed to create lead." },
        { status: 500 }
      );
    }
    ...
    // 4) Save report to new `reports` table.
    ...
    if (reportErr || !report?.id) {
      return NextResponse.json(
        {
          success: false,
          message: reportErr?.message || "Failed to create property report.",
        },
        { status: 500 }
      );
    }

-- Revenue Dashboard + KPI system: events, revenue, alerts, funnel analytics
-- agent_id type MUST match public.agents.id (uuid OR bigint — detected at migrate time).
...
begin
  ...
  if v_agent_type is null then
    raise notice 'public.agents not found; skipping revenue KPI tables';
    return;
  end if;

  if v_agent_type not in ('uuid', 'bigint') then
    raise exception 'Unsupported public.agents.id type for revenue KPI: %', v_agent_type;
  end if;

  if v_agent_type = 'uuid' then
    execute $sql$
      create table if not exists public.agent_business_events (
...
create unique index if not exists revenue_transactions_external_ref_unique
  on public.revenue_transactions (external_ref)
  where external_ref is not null;

create index if not exists idx_agent_business_events_agent_created
  on public.agent_business_events (agent_id, created_at desc);

create index if not exists idx_agent_business_events_agent_name_created
  on public.agent_business_events (agent_id, event_name, created_at desc);

create index if not exists idx_agent_business_events_session
  on public.agent_business_events (agent_id, session_id, created_at desc);

create index if not exists idx_revenue_transactions_agent_occurred
  on public.revenue_transactions (agent_id, occurred_at desc);

create index if not exists idx_kpi_alert_rules_agent
  on public.kpi_alert_rules (agent_id, enabled);

create unique index if not exists kpi_alert_rules_agent_metric_unique
  on public.kpi_alert_rules (agent_id, metric_key);

create index if not exists idx_kpi_alert_events_agent_created
...
-- RLS: authenticated agents read own rows; writes via service role API only

...
import { hashPrompt, getCachedResponse, setCachedResponse } from "@/lib/ai/cache";
 ...
 * Temperature default 0.7 per spec. Returns plain text only.
 */
export async function generateAIResponse(input: GenerateAIResponseInput): Promise<{
  ...
  if (!text && lastErr) {
    console.error("[PropertyTools AI] generateAIResponse failed", lastErr.message);

 ...
 *   npm run smoke:lead-score -w property-tools -- <lead-uuid>
 *
 * Or from apps/property-tools:
 *   npx tsx scripts/smoke-lead-score.ts <lead-uuid>
 *
 * Env: apps/property-tools/.env.local must define:
...
async function main() {
  ...
  if (!leadId) {
    console.error(
      "Usage: npx tsx scripts/smoke-lead-score.ts <lead-uuid>\n   or: LEAD_ID=<uuid> npm run smoke:lead-score -w property-tools"
    );
    process.exitCode = 1;
    return;
  }

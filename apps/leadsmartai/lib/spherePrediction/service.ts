...
/**
 * Service layer for the SOI ("sphere of influence") seller-prediction workflow.
 *
 * Public method `topLikelySellersForAgent` powers the daily "Today's likely
 * sellers" dashboard panel — it pulls past_client + sphere contacts for an
 * agent, scores each via the rules engine, and returns the top-N ranked.
 *
 * Performance note: scoring is computed on-demand (not persisted). The query
 * fetches only the columns needed and caps at LIMIT_CANDIDATES to keep memory
 * bounded. For agents with >2K contacts we'll add a persistence layer later
 * (separate column `sphere_seller_score` like `prediction_score` on
 * dealPrediction) but YAGNI until we see a slow query.
 *
 * Error handling: missing tables / columns degrade to empty results rather
 * than throwing, matching the pattern in lib/contacts/service.ts. The
 * dashboard panel renders a friendly empty state in that case.
 */

const LIFECYCLE_STAGES: LifecycleStage[] = ["past_client", "sphere"];

/** Hard cap on candidates loaded per agent — keeps query memory bounded. */
const LIMIT_CANDIDATES = 2_000;

export type LikelySellerRow = {
  contactId: string;
  fullName: string;
  email: string | null;
  phone: string | null;
  ...
  /** Top reason — first factor with the highest pointsEarned. UI uses this for the one-liner. */
  topReason: string;
  /** Full factor breakdown for drill-down / hover tooltips. */
  factors: SphereSellerScoreResult["factors"];
};
...
function topReasonOf(result: SphereSellerScoreResult): string {
  // Pick the factor with the highest pointsEarned. Tie-break by id order
  // (which mirrors weight order — tenure first, etc.) so output is stable.
...
/**
 * Top-N likely sellers for an agent, sorted by score desc.
 *
 * @param agentId  Owner of the contacts to score.
 * @param opts.limit       How many to return. Default 25. UI caps at 100.
 * @param opts.minScore    Skip contacts below this score. Default 0 (no floor).
 * @param opts.label       Optional filter — only `high` / `medium` / `low`.
 */
export async function topLikelySellersForAgent(
  agentId: string,
  opts: { limit?: number; minScore?: number; label?: SphereSellerLabel } = {},
): Promise<LikelySellerRow[]> {
  ...
  // Filter out automation_disabled rows here (cheaper than in SQL via
  // ".eq('automation_disabled', false)" because the column may be null).
  const candidates = (contactRows as unknown as ContactRow[]).filter(
    (r) => r.automation_disabled !== true,
  );
  if (candidates.length === 0) return [];

  // Pull open signals for the candidate set — bulk lookup keeps it to one query.

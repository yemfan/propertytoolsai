 ...
 *
 * Usage:
 *   npm run smoke:stripe-test-card -w leadsmartai
 *
 * Env: STRIPE_SECRET_KEY=sk_test_... in apps/leadsmartai/.env.local
 */
...
async function main() {
  ...
  if (!key) {
    console.error("Missing STRIPE_SECRET_KEY (set in apps/leadsmartai/.env.local)");
    process.exitCode = 1;
    return;
  }

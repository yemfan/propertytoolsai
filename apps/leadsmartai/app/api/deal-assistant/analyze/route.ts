...
export async function POST(req: Request) {
  try {
    ...
    if (!(await userHasCrmFeature(user.id, "full_ai"))) {
      return subscriptionRequiredResponse("full_ai", "crm_full_ai_locked");
    }
    ...
    if (!(await peekAiUsageAllowed(user.id))) {
      return subscriptionRequiredResponse("full_ai", "ai_usage_limit_reached");
    }
    ...
    const credit = await tryConsumeAiCredit(user.id, {
      usageMetadata: { surface: "deal_assistant" },
    });
    if (!credit.allowed) {
      return subscriptionRequiredResponse("full_ai", "ai_usage_limit_reached");
    }

...
import { dispatchMobileReminderPush } from "@/lib/mobile/pushDispatch";
...
export async function GET() {
  try {
    ...
    for (const lead of (leads as any[]) ?? []) {
      ...
      if (agentId) {
        void dispatchMobileReminderPush({
          agentId,
          leadId,
          leadName: lead.name != null ? String(lead.name) : null,
          hint: lead.property_address != null ? String(lead.property_address).slice(0, 80) : undefined,
        }).catch(() => {});
      }

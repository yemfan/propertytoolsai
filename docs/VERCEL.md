...
3. **Heap / OOM** — Both apps prerender many static routes (`leadsmart-ai` ~1500+ pages, `property-tools` ~2500+). Each app’s `next.config.js` lowers **static generation concurrency** and enables **webpack memory optimizations** to reduce peak RSS. `apps/*/vercel.json` sets **12 GiB** `NODE_OPTIONS` for the build step when Root Directory is that app. The **root** `package.json` scripts set `NODE_OPTIONS` for **`turbo build`** and per-app builds. You can mirror **`NODE_OPTIONS`** in **Project → Settings → Environment Variables** if needed.

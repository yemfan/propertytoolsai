...
3. **Heap / OOM** — Both apps prerender many static routes (`property-tools` is especially heavy: ~2500+ pages). `apps/property-tools/next.config.js` lowers **static generation concurrency** and enables **webpack memory optimizations** to reduce peak RSS. `apps/*/vercel.json` sets a larger `NODE_OPTIONS` heap for the build step when Root Directory is that app (`property-tools` uses **12 GiB**). The **root** `package.json` scripts set `NODE_OPTIONS` for **`turbo build`** (12 GiB) and per-app builds. You can mirror **`NODE_OPTIONS`** in **Project → Settings → Environment Variables** if needed.
